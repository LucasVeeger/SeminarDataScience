...
author: "Sara Hacipoglu (5569206), Lucas (4459628)"
date: "24/03/2022"
output:
  html_document:
    df_print: paged
...
![Conceptual model Twitter sentiment analysis](C:/Users/mirij/Downloads/question2.png)
...
# fixed effect
# session added as random slope
scoreRI <-update(randomIntercept, .~. + session)
# session now conditioned on person
scoreRS <-update(scoreRI, random = ~session|subject)

ARModel <- update(scoreRS, correlation = corAR1(0, form = ~session|subject))

anova(randomIntercept, scoreRI, scoreRS, ARModel)
summary(ARModel)
# 
...
```
SessionRI has the lowest AIC value and p-value compared to the randomIntercept.
The Phi value from the Correlation Structure shows that session is significant for score value.
