...
From the boxplot we can see that for all three topics, most of the tweets have a sentiment around 0.
...
#CrossTable(semFrame$score, semFrame$Candidate, prop.r=FALSE, prop.c = FALSE,
#prop.t = FALSE,
...
# Model0 <- lm(score ~ 1, data = lec7dsub)
# model1 <- lm(score ~ Feedback, data = lec7dsub)
# anova(model0,model1, test = "F")
      
#From markdown
# For model without predictor, should we import the same amount of tweets but not related to the topics of Energy transition and then make a comparison?
model0<- lm(score ~ 1, data = semFrame) #model without predictor
model1 <- lm(score ~ Candidate, data = semFrame) #model with predictor

# model2 <- lm(score ~ Candidate, data = sustainableEnergy) #model with predictor
# model3 <- lm(score ~ Candidate, data = renewableEnergy) #model with predictor
# model4 <- lm(score ~ Candidate, data = energyTransition) #model with predictor
# 
# pander(anova(model0, model2, test = 'F'),
# caption = "Compare if sustainableEnergy provide better fit than no knowledge")
# pander(anova(model0, model3, test = 'F'),
# caption = "Compare if renewableEnergy provide better fit than no knowledge")
# pander(anova(model0, model4, test = 'F'),
# caption = "Compare if energyTransition provide better fit than no knowledge")
...
```
The P value is significantly smaller than 0.05, thus we can accept the null hypothesis. Thus, the organisation gives insight into the sentiment.
From the last column in the summary model, we see that the p-value for sustainableEnergy and renewableEnergy is larger than 0.05. Thus the organisation has significant impact on the output score.
...
# 
# fit1<- bayes.t.test(renewableEnergy$score,energyTransition$score)
# show(fit1)
# plot(fit1)

# fit2 <- bayes.t.test(renewableEnergy$score,sustainableEnergy$score)
# show(fit2)
# plot(fit2)

# fit3 <- bayes.t.test(energyTransition$score,sustainableEnergy$score)
# show(fit3)
# plot(fit3)
```
...
Model that includes parameter estimation for the organisations:
```{r, warning=FALSE}
m1 <-map2stan(
  alist(
  score ~ dnorm(mu, sigma),
